...
export default function PayrollFilters({ filters, onChange }) {
  ...
  const statusOptions = [
    ...
    { value: "debit", label: "مستحق له (مدين)" },
    { value: "credit", label: "مستقطع منه / عليه (دائن)" },
  ];
  ...
  return (
    <div className="bg-slate-200/50 backdrop-blur-sm rounded-xl border border-slate-300/40 p-3 mb-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
        ...
        {/* من تاريخ */}
        <div>
          <label className={fieldLabelCls}>تاريخ بداية (من)</label>
          <input
            type="date"
        ...
        {/* إلى تاريخ */}
        <div>
          <label className={fieldLabelCls}>تاريخ نهاية (إلى)</label>
          <input
            type="date"
        ...
        <div className="relative z-30">
          <label className={fieldLabelCls}>حالة الحركة</label>
          <CompactSelect
            options={statusOptions}
            value={filters?.status || "all"}
            onChange={(val) => set("status", val || "all")}
