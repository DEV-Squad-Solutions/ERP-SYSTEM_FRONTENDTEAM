...
const HOME_ROLES = ["Admin", "Accountant"];
...
const getDefaultRoute = (roles = []) => {
  ...
  const hasHomeAccess = HOME_ROLES.some((role) =>
    normalizedRoles.includes(role.toLowerCase()),
  );

  return hasHomeAccess ? "/dashboard" : "/dashboard/sales";
};

export default function LoginPage() {
  const navigate = useNavigate();

  const { requiresCompanySelection, isAuthenticated, roles } = useSelector(
    (state) => state.auth,
  );
