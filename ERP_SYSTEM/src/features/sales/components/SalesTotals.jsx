...
export default function SalesTotals({ filters }) {
  ...
  // خصم/ضريبة/مدفوع مرة واحدة لكل فاتورة
  const uniqueInvoices = [
    ...new Map(rows.map((r) => [r.invoiceNumber ?? r.id, r])).values(),
  ];

  const discount = uniqueInvoices.reduce(
    (sum, r) => sum + Number(r.invoiceDiscount ?? 0),
    0,
  );

  const tax = uniqueInvoices.reduce(
    (sum, r) => sum + Number(r.invoiceTax ?? 0),
    0,
  );

  const paid = uniqueInvoices.reduce(
    (sum, r) => sum + Number(r.invoicePaid ?? 0),
    0,
  );
