import { Edit3, Printer, Trash2, Wallet } from "lucide-react";

export default function TreasuryTable({ transactions = [], isLoading, onEdit, onDelete, onPrintRow }) {
  if (isLoading) {
    return (
  ...
  if (!transactions || transactions.length === 0) {
    return (
      <div className="text-center py-14 border border-dashed border-ink-400/20 rounded-2xl bg-white">
        <Wallet size={32} className="mx-auto text-ink-400/40 mb-2" />
        <p className="text-ink-400 text-sm">لا توجد حركات مسجلة</p>
      </div>
    );
  }

  // حساب الرصيد المتراكم والإجماليات
      ...
      <table className="w-full text-right border-collapse min-w-[1000px]">
        <thead>
          <tr className="bg-ink-900/[0.03] text-ink-400 text-xs">
            ...
            <th className="p-3 font-medium border-b border-ink-400/10 text-center w-28 text-positive">مدين (إيداع)</th>
            <th className="p-3 font-medium border-b border-ink-400/10 text-center w-28 text-negative">دائن (صرف)</th>
            <th className="p-3 font-medium border-b border-ink-400/10 text-center w-32">الرصيد</th>
        ...
        <tfoot>
          <tr className="bg-primary-50/50 border-t-2 border-primary-100 font-semibold text-ink-900">
            <td className="p-3.5" colSpan={2}>الرصيد الإجمالي</td>
